...
def get_outstanding_dev_tasks(api, work_type):
    ...
    for (project_id, project_name) in project_info:
        ...
        root_tasks = [
            t for t in project_data['items'] if label_id in t['labels'] and not t['parent_id']
        ]
...
def create_pr_checklist_subtask(api, project_id, root_task_id, branch, task_type, work_type):
    ...
    create_subtask(
        api, 'Create final commit', project_id, task_type, work_type, pr_checklist_subtask_id)
    create_subtask(
        api,
        ...
        pr_checklist_subtask_id)
    create_subtask(
        api, 'Carefully proof read commits', project_id, task_type, work_type, pr_checklist_subtask_id)
    create_subtask(
        api,
...
def create_merge_subtasks(api, project_id, root_task_id, branch, repo, target, task_type, work_type):
    create_subtask(
        api, f'Submit pull request for `{branch}`', project_id, task_type, work_type, root_task_id)
    create_subtask(
        api,
...
def create_jira_admin_task(api, project_id, root_task_id, jira_ref):
    create_subtask(
        api, f'Close `{jira_ref}`', project_id, DevTaskType.ADMIN, DevWorkType.WORK, root_task_id)
